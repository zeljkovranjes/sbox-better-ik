# bik editor-process regression gate driver.
#
# Creates a scratch s&box game project OUTSIDE the repo, installs this library into it via
# NTFS junction, launches sbox-dev.exe on it, and waits for the in-editor hook
# (Editor/BetterIk/BikGate.cs, armed by BIK_GATE_RESULT) to write a JSON result file.
# Reports pass/fail from that JSON. Mirrors humanoid-retargeter/dev/editor-rig/run_editor_gate.ps1.
#
# Usage:
...
$scratch    = Join-Path $env:TEMP "bik-editor-rig\scratch"  # OUTSIDE the repo: a scratch project inside the library tree creates a junction CYCLE (lib -> repo -> scratch -> lib) that intermittently crashes the s&box editor when this repo is junctioned into other projects
$sbproj     = Join-Path $scratch "bikscratch.sbproj"
$libDir     = Join-Path $scratch "Libraries\local.better_ik"
$resultPath = Join-Path $PSScriptRoot "bik_result.json"
...
if (-not (Test-Path $sbproj)) {
    ...
    $proj = $proj -replace '"Title":\s*"[^"]*"', '"Title": "BIK Scratch"'
    $proj = $proj -replace '"Ident":\s*"[^"]*"', '"Ident": "bikscratch"'
...
if ($CopyLibrary) {
    ...
    robocopy "$repoRoot" "$libDir" /MIR /XD .git .claude dev docs research /XF *.user /NFL /NDL /NJH /NJS | Out-Null
...
# One-shot arming marker: the in-editor gate only runs when this file exists (and consumes
# it). Protects real sessions from a leaked BIK_GATE_RESULT env var - Steam booted as a gate
# child inherits the var and passes it to every editor launch afterwards.
...
# --- launch ----------------------------------------------------------------------
$env:BIK_GATE_RESULT = $resultPath
try {
    ...
    Remove-Item Env:BIK_GATE_RESULT -ErrorAction SilentlyContinue
}
...
if (Test-Path $sboxLog) {
    ...
    try {
        ...
        if ($fs.Length -gt $preLogLen) {
            ...
            $interesting = $newLog | Where-Object { $_ -match 'bik-gate|better_ik|betterik|Error' }
...
Write-Host ""
Write-Host "===== bik_result.json =====" -ForegroundColor Cyan
...
if ($res.passed) {
    Write-Host "RESULT: PASS - all $($res.scenarios.Count) scenarios passed." -ForegroundColor Green
    exit 0
}
$failedNames = ($res.scenarios | Where-Object { -not $_.passed } | ForEach-Object { "$($_.name): $($_.failReason)" }) -join "; "
Fail 1 "FAIL - $failedNames"
